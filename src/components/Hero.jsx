...
import CompnyLogos from "./CompnyLogos";
import CurvedGradientLine from "./CurvedGradientLine";
const Hero = () => {
  ...
  return (
    ...
    >
      <div ref={parallexRef} className="container relative">
                ...
                <ScrollParallax isAbsolutelyPositioned>
                  <ul className="hidden absolute -left-[5.5rem] bottom-[7.5rem] px-1 py-1 bg-n-9/40 backdrop-blur border border-n-1/10 rounded-2xl xl:flex">
                    {heroIcons.map((icon, i) => (
                      <li key={i} className="p-5 hover:bg-n-10 rounded-2xl cursor-pointer duration-150 ">
        ...
        <CompnyLogos className="hidden relative z-10 mt-20 lg:block" />
      </div>
      <BottomLine />
    </Section>
  );
